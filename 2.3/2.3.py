...
def mixed_exponential_weighted_covariance(data, variance_lambda=0.97, correlation_lambda=0.94):
    variance_cov_matrix = exponential_weighted_covariance(data, variance_lambda)
    variances = np.diag(variance_cov_matrix)
    std_devs_variance = np.sqrt(variances)

    corr_matrix = exponential_weighted_correlation(data, correlation_lambda)

    m = len(variances)
    mixed_cov_matrix = np.zeros((m, m))

    for i in range(m):
        for j in range(m):
            if i == j:
                # Diagonal elements
                mixed_cov_matrix[i, j] = variances[i]
            else:
                # Off-diagonal elements
                mixed_cov_matrix[i, j] = corr_matrix[i, j] * std_devs_variance[i] * std_devs_variance[j]

    return mixed_cov_matrix

def main():
    data = pd.read_csv('test2.csv')

    mixed_cov = mixed_exponential_weighted_covariance(data, variance_lambda=0.97, correlation_lambda=0.94)

    print(','.join(data.columns))

    for row in mixed_cov:
